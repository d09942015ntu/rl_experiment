...
def addition_steps(a, b, n=3):
    ...
    while len(digits_a) < n:
        digits_a = [0] + digits_a
    while len(digits_b) < n:
        digits_b = [0] + digits_b
    ...
    for i in range(n-1, -1, -1):  # Indices from 3 to 0
...
def save_dataset(used,n,seed=0):
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        writer.writerow([f's{i}' for i in range(n+2)])
...
if __name__ == '__main__':
    used=set()
    save_dataset(used,3,0)
    save_dataset(used,3,1)
    print(used)
